...
{
    iconDraw(id, isBombActive(id) ? true : false)
    return PLUGIN_CONTINUE
}
    ...
    switch( item )
    {
        ...
        case TOGGLE_CURRENT:
        {
            ...
            if ( g_eSettings[SETTING_ICON_SHOW] )
                iconColor(eBomb[BOMB_ICON], eBomb[BOMB_STATE] == STATE_ACTIVE ? true : false)
...
public menuHandlerRemove(id, menu, item)
{
    ...
    switch( item )
    {
        ...
        case REMOVE_CURRENT:
        {
            ...
            iArg[TASK_TYPE] = g_iBomb > 0 ? MENU_REMOVE : MENU_ROOT
            set_task(MENU_BLINK, "bombMenu", .parameter = iArg, .len = sizeof(iArg))
        }
        case REMOVE_ALL:
        {
...
public menuHandlerScale(id, menu, item)
{
    ...
    switch( item )
    {
        ...
        {
            eBomb[BOMB_SIZE_Z] += 5.0

            if (eBomb[BOMB_SIZE_Z] > g_eSettings[SETTING_SIZE_HEIGHT_MAX])
                eBomb[BOMB_SIZE_Z] = g_eSettings[SETTING_SIZE_HEIGHT_MAX]
        ...
        case SCALE_HEIGHT_SUB:
        {
            ...
            if (eBomb[BOMB_SIZE_Z] < g_eSettings[SETTING_SIZE_HEIGHT_MIN])
                eBomb[BOMB_SIZE_Z] = g_eSettings[SETTING_SIZE_HEIGHT_MIN]
            ArraySetArray(g_aBomb, iItem, eBomb)
        ...
        {
            eBomb[BOMB_SIZE_X] += 5.0

            if (eBomb[BOMB_SIZE_X] > g_eSettings[SETTING_SIZE_WIDTH_MAX])
                eBomb[BOMB_SIZE_X] = g_eSettings[SETTING_SIZE_WIDTH_MAX]
            eBomb[BOMB_SIZE_Y] = eBomb[BOMB_SIZE_X]
            ArraySetArray(g_aBomb, iItem, eBomb)
        ...
        case SCALE_WIDTH_SUB:
        {
            ...
            if (eBomb[BOMB_SIZE_X] < g_eSettings[SETTING_SIZE_WIDTH_MIN])
                eBomb[BOMB_SIZE_X] = g_eSettings[SETTING_SIZE_WIDTH_MIN]
            eBomb[BOMB_SIZE_Y] = eBomb[BOMB_SIZE_X]
            ArraySetArray(g_aBomb, iItem, eBomb)
...
stock bombCreate(id, iItem = -1)
{
    ...
    else
    {
        ...
        eBomb[BOMB_ID] = iEnt
        eBomb[BOMB_STATE] = STATE_INACTIVE
        eBomb[BOMB_SIZE_X] = eBomb[BOMB_SIZE_Y] = eBomb[BOMB_SIZE_Z] = g_eSettings[SETTING_SIZE_BASE]
        dllfunc(DLLFunc_Spawn, iEnt)
    }
...
stock iconRefresh()
{
    ...
    for ( new i = 0; i < iNum; i ++ )
        iconDraw(iPlayers[i], isBombActive(iPlayers[i]) ? true : false)
}
...
public saveData(id)
{
    ...
    for ( new i = 0; i < g_iBomb; i ++ )
    {
        ...
        formatex(szData, charsmax(szData), "state = %d^n", eBomb[BOMB_STATE])
        fputs(iFile, szData)
...
{
    eBomb[BOMB_CORNERS][0]  = eBomb[BOMB_ORIGIN][0] - eBomb[BOMB_SIZE_X]
    eBomb[BOMB_CORNERS][1]  = eBomb[BOMB_ORIGIN][1] - eBomb[BOMB_SIZE_Y]
    eBomb[BOMB_CORNERS][2]  = eBomb[BOMB_ORIGIN][2] - eBomb[BOMB_SIZE_Z]

    eBomb[BOMB_CORNERS][3]  = eBomb[BOMB_ORIGIN][0] + eBomb[BOMB_SIZE_X]
    eBomb[BOMB_CORNERS][4]  = eBomb[BOMB_ORIGIN][1] - eBomb[BOMB_SIZE_Y]
    eBomb[BOMB_CORNERS][5]  = eBomb[BOMB_ORIGIN][2] - eBomb[BOMB_SIZE_Z]

    eBomb[BOMB_CORNERS][6]  = eBomb[BOMB_ORIGIN][0] - eBomb[BOMB_SIZE_X]
    eBomb[BOMB_CORNERS][7]  = eBomb[BOMB_ORIGIN][1] + eBomb[BOMB_SIZE_Y]
    eBomb[BOMB_CORNERS][8]  = eBomb[BOMB_ORIGIN][2] - eBomb[BOMB_SIZE_Z]

    eBomb[BOMB_CORNERS][9]  = eBomb[BOMB_ORIGIN][0] + eBomb[BOMB_SIZE_X]
    eBomb[BOMB_CORNERS][10] = eBomb[BOMB_ORIGIN][1] + eBomb[BOMB_SIZE_Y]
    eBomb[BOMB_CORNERS][11] = eBomb[BOMB_ORIGIN][2] - eBomb[BOMB_SIZE_Z]

    eBomb[BOMB_CORNERS][12] = eBomb[BOMB_ORIGIN][0] - eBomb[BOMB_SIZE_X]
    eBomb[BOMB_CORNERS][13] = eBomb[BOMB_ORIGIN][1] - eBomb[BOMB_SIZE_Y]
    eBomb[BOMB_CORNERS][14] = eBomb[BOMB_ORIGIN][2] + eBomb[BOMB_SIZE_Z]

    eBomb[BOMB_CORNERS][15] = eBomb[BOMB_ORIGIN][0] + eBomb[BOMB_SIZE_X]
    eBomb[BOMB_CORNERS][16] = eBomb[BOMB_ORIGIN][1] - eBomb[BOMB_SIZE_Y]
    eBomb[BOMB_CORNERS][17] = eBomb[BOMB_ORIGIN][2] + eBomb[BOMB_SIZE_Z]

    eBomb[BOMB_CORNERS][18] = eBomb[BOMB_ORIGIN][0] - eBomb[BOMB_SIZE_X]
    eBomb[BOMB_CORNERS][19] = eBomb[BOMB_ORIGIN][1] + eBomb[BOMB_SIZE_Y]
    eBomb[BOMB_CORNERS][20] = eBomb[BOMB_ORIGIN][2] + eBomb[BOMB_SIZE_Z]

    eBomb[BOMB_CORNERS][21] = eBomb[BOMB_ORIGIN][0] + eBomb[BOMB_SIZE_X]
    eBomb[BOMB_CORNERS][22] = eBomb[BOMB_ORIGIN][1] + eBomb[BOMB_SIZE_Y]
    eBomb[BOMB_CORNERS][23] = eBomb[BOMB_ORIGIN][2] + eBomb[BOMB_SIZE_Z]
}
...
{
    return engfunc(EngFunc_FindEntityByString, -1, "classname", "info_bomb_target") > 0
